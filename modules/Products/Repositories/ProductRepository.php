...
class ProductRepository implements ProductInterface
{
    ...
    {
        try {
            $products = Product::with(['images','vendor'])->simplePaginate(10);
     ...
     *    @OA\JsonContent(
     *           required={"term"},
     *          @OA\Property(property="term", type="string", example = "samsung"),
     *    ),
     * ),
     * @OA\Response(
     *     response=200,
    ...
    {
        try {
            $products = Product::with(['images','vendor'])->where('name', 'Like', '%' . $request->term . '%')->orderBy('id', 'DESC')->simplePaginate(10);
            if (is_null($products)) {
                return $this->ApiResponse(Response::HTTP_NOT_FOUND, null,"No Products found");
            }
     ...
     *          @OA\Property(property="specs", type="array",
     *          @OA\Items(
     *                   @OA\Property(property="value", type="string"),
     *                  @OA\Property(property="spec_id", type="integer")
     *              ),
     *           ),
     *      ),
     * ),
    ...
    public function show(Product $product)
    {
        ...
        $product = Product::with(['images','vendor','reviews','rates','specs'])->find($product->id);
    ...
    {
        return $this->ApiResponse(Response::HTTP_NOT_FOUND, null, 'THIS PRODUCT NOT EXIST.');
    }
